...
import static p8.demo.p8sokoban.State.Global.customHandler;
...
import static p8.demo.p8sokoban.State.Global.milliseconds;
...
import static p8.demo.p8sokoban.State.Global.timeSwapBuff;
...
public class p8_Sokoban extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(difficultystate==1){
            mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.bbcf_grim_of_abyss_hub);
            mediaPlayer.start();
            DifficultyChoose();
        }
        else if(difficultystate==2){
            mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.subterranean_canal);
            mediaPlayer.start();
            DifficultyChoose();
        }
        else if(difficultystate==3){
            mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.t3b_unknown_unknown);
            mediaPlayer.start();
            DifficultyChoose();
        }
    ...
    public void ClickNumber(String str) {
        number_pressed = 1;
        event_number = Integer.parseInt(str);
        number_pressed = 0;
    }
